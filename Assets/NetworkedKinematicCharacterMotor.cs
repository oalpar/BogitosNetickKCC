...
namespace BogitosKCC
{
	...
	public struct KinematicCharacterMotorState
	{
		...
		public NetworkTimer MustUngroundTime;
		public bool LastMovementIterationFoundAnyGround;
		public CharacterTransientGroundingReport GroundingStatus;

		public NetworkBehaviourRef<NetworkedPhysicsMover> PhysicsMoverRef;
		...
		public override string ToString()
		{
			...
			return $"Position={Position}, " +
				   ...
				   $"AttachedRigidbody={(AttachedRigidbody != null ? AttachedRigidbody.name : "null")}, " +
				   $"PhysicsMoverRef={PhysicsMoverRef.BehaviourId}, " +
				   $"AttachedRigidbodyVelocity={AttachedRigidbodyVelocity}";
		}


		public static bool operator ==(KinematicCharacterMotorState state1, KinematicCharacterMotorState state2)
		{
			const float tolerance = 0.01f;

			return Vector3.Distance(state1.Position, state2.Position) < tolerance &&
				   Quaternion.Angle(state1.Rotation, state2.Rotation) < tolerance &&
				   Vector3.Distance(state1.BaseVelocity, state2.BaseVelocity) < tolerance &&
				   state1.MustUnground == state2.MustUnground &&
				   state1.MustUngroundTime.Equals(state2.MustUngroundTime) &&
				   state1.LastMovementIterationFoundAnyGround == state2.LastMovementIterationFoundAnyGround &&
				   state1.GroundingStatus.Equals(state2.GroundingStatus) &&
				   Equals(state1.AttachedRigidbody, state2.AttachedRigidbody) &&
				   Vector3.Distance(state1.AttachedRigidbodyVelocity, state2.AttachedRigidbodyVelocity) < tolerance;
		}
		...
		{
			if (obj is KinematicCharacterMotorState state)
			{
				return this == state;
			}
			return false;
		...
		public override int GetHashCode()
		{
			...
			hash = (hash * 397) ^ BaseVelocity.GetHashCode();
			hash = (hash * 397) ^ MustUnground.GetHashCode();
			hash = (hash * 397) ^ MustUngroundTime.GetHashCode();
			hash = (hash * 397) ^ LastMovementIterationFoundAnyGround.GetHashCode();
			hash = (hash * 397) ^ GroundingStatus.GetHashCode();
			hash = (hash * 397) ^ (AttachedRigidbody != null ? AttachedRigidbody.GetHashCode() : 0);
			hash = (hash * 397) ^ AttachedRigidbodyVelocity.GetHashCode();
			return hash;
		}
	}
	...
	public struct CharacterGroundingReport
	{
		...
		public int ColliderIndexInMover;
		...
		public void CopyFrom(CharacterTransientGroundingReport transientGroundingReport)
		{
			...
			ColliderIndexInMover = transientGroundingReport.ColliderIndexInMover;
			GroundCollider = null;
	...
	public struct CharacterTransientGroundingReport
	{
		...
		public void CopyFrom(CharacterGroundingReport groundingReport)
		{
			...
			ColliderIndexInMover = groundingReport.ColliderIndexInMover;
		}

		public override string ToString()
		{
			return $"FoundAnyGround={FoundAnyGround}, " +
	...
	[RequireComponent(typeof(KCCMotorPhase2))]
	[RequireComponent(typeof(KCCMotorPhase1))]
	[RequireComponent(typeof(CapsuleCollider))]
	public class NetworkedKinematicCharacterMotor : NetworkBehaviour
	{

		[Networked(precision: 0.000001f)]
		[Smooth]
		public KinematicCharacterMotorState networkedState
		{
			get; set;
		}

		[Networked(ensureSafety: true)]
		[Smooth]
		...
		{
			smoothedRot = Quaternion.identity;
		}
		...
		public override void NetworkRender()
		{
			if (RenderTransform == null)
				return;
			RenderTransform.position = smoothedPos + new Vector3(0, CapsuleYOffset, 0);
			RenderTransform.rotation = smoothedRot.normalized;
		}

		public override void NetcodeIntoGameEngine()
		{
			ApplyState(networkedState);
			smoothedPos = networkedState.Position;
			smoothedRot = networkedState.Rotation;
		}

		public override void GameEngineIntoNetcode()
		{
			networkedState = GetState();
			//_attachedRigidbody = networkedState.AttachedRigidbody;
			smoothedPos = networkedState.Position;
			smoothedRot = networkedState.Rotation;
		}
...
#pragma warning restore 0649
		...
		/// </summary>
		[NonSerialized] public KinematicCharacterController.ICharacterController CharacterController;
		/// <summary>
		...
		private RigidbodyProjectionHit[] _internalRigidbodyProjectionHits =
			new RigidbodyProjectionHit[MaxRigidbodyOverlapsCount];
		...
		private NetworkTimer _mustUngroundTimeCounter;
...
#pragma warning restore 0414
		...
		public void Reset()
		{
			ApplyState(new KinematicCharacterMotorState
			{
				Position = Vector3.zero,
				Rotation = Quaternion.identity,
				PhysicsMoverRef = new(null),
		...
		public void OnDrawGizmos()
		{
			...
			Vector3 velocityEnd = transform.position + Velocity;
			DrawAxisWithCone(transform.position, velocityEnd, Color.red);
			if (Velocity.magnitude > 0f)
				Handles.Label(velocityEnd, $"Velocity={Velocity}");
			...
			// Compute world space directions from the quaternion
			Vector3 right = rot * Vector3.right;   // X-axis
			Vector3 up = rot * Vector3.up;         // Y-axis
			Vector3 forward = rot * Vector3.forward; // Z-axis

			// Draw axes with cones
			DrawAxisWithCone(transform.position, transform.position + right, Color.red);   // X-axis
			DrawAxisWithCone(transform.position, transform.position + up, Color.green);    // Y-axis
			DrawAxisWithCone(transform.position, transform.position + forward, Color.blue); // Z-axis

			if (_transform != null)
				Handles.Label(Transform.position, $"Transform Up={transform.up},\n" +
		...
		private void DrawAxisWithCone(Vector3 start, Vector3 end, Color color)
		{
			// Draw the axis line
			Gizmos.color = color;
			...
			{
				Handles.ConeHandleCap(0, end, Quaternion.LookRotation(direction), 0.2f, EventType.Repaint);
			}
			else
			{
		...
		public KinematicCharacterMotorState GetState()
		{
			...
			{
				var mover = _attachedRigidbody.transform.GetComponent<NetworkedPhysicsMover>();

				if (mover != null)
				{
					state.PhysicsMoverRef = mover.GetRef<NetworkedPhysicsMover>();
				}
				else
				{
					state.PhysicsMoverRef = new(null);
				}

				var index = Array.IndexOf(mover.Colliders, GroundingStatus.GroundCollider);
				state.GroundingStatus.ColliderIndexInMover = index;
			}
			else
			{
				state.PhysicsMoverRef = new(null);
		...
		public void ApplyState(KinematicCharacterMotorState state, bool bypassInterpolation = true)
		{
			...
			var NO = state.PhysicsMoverRef.GetBehaviour(Sandbox);
			if (NO != null)
			{
				_attachedRigidbody = NO.GetComponent<Rigidbody>();
				var index = state.GroundingStatus.ColliderIndexInMover;
				GroundingStatus.GroundCollider = NO.Colliders[index];
			}
			else
			{
						...
						for (int i = 0; i < nbOverlaps; i++)
						{
							if (GetInteractiveRigidbody(_internalProbedColliders[i]) == null)
							{
										...
										out resolutionDistance))
								{
									...
									KinematicCharacterController.HitStabilityReport mockReport = new KinematicCharacterController.HitStabilityReport();
			...
			if (_solveGrounding)
			{
				...
				else
				{
					...
					if (!LastGroundingStatus.IsStableOnGround && GroundingStatus.IsStableOnGround)
					{
						...
						BaseVelocity = GetDirectionTangentToSurface(BaseVelocity, GroundingStatus.GroundNormal) *
							...
							for (int i = 0; i < nbOverlaps; i++)
							{
										...
										out resolutionDistance))
								{
									...
									if (InteractiveRigidbodyHandling)
									{
										...
										{
											KinematicCharacterController.HitStabilityReport tmpReport = new KinematicCharacterController.HitStabilityReport();
		...
		/// Determines if motor can be considered stable on given slope normal
		/// </summary>
		private bool IsStableWithSpecialCases(ref KinematicCharacterController.HitStabilityReport stabilityReport, Vector3 velocity)
		{
			...
			ref CharacterGroundingReport groundingReport)
		{
			...
			while (groundProbeDistanceRemaining > 0 && (groundSweepsMade <= MaxGroundingSweepIterations) && !groundSweepingIsOver)
			{
						...
						out groundSweepHit)) // hit
				{
					...
					KinematicCharacterController.HitStabilityReport groundHitStabilityReport = new KinematicCharacterController.HitStabilityReport();
		...
		{
			_mustUnground = true;
			_mustUngroundTimeCounter = NetworkTimer.StartTimer(time, Sandbox.Engine, true);
		}

		public bool MustUnground()
		{
			return _mustUnground || _mustUngroundTimeCounter.TargetTick >= Sandbox.Tick;
		}
		...
		private bool InternalCharacterMove(ref Vector3 transientVelocity, float deltaTime)
		{
				   ...
				   hitSomethingThisSweepIteration)
			{
				...
				if (foundClosestHit)
				{
					...
					KinematicCharacterController.HitStabilityReport moveHitStabilityReport = new KinematicCharacterController.HitStabilityReport();
						...
						moveHitStabilityReport.ValidStepDetected)
					{
						...
						if (obstructionCorrelation <= CorrelationForVerticalObstruction)
						{
							Vector3 stepForwardDirection =
								Vector3.ProjectOnPlane(-closestSweepHitNormal, _characterUp).normalized;
							Vector3 stepCastStartPoint =
								(tmpMovedPosition + (stepForwardDirection * SteppingForwardDistance)) +
		...
		private void StoreRigidbodyHit(Rigidbody hitRigidbody, Vector3 hitVelocity, Vector3 hitPoint,
			Vector3 obstructionNormal, KinematicCharacterController.HitStabilityReport hitStabilityReport)
		{
			if (_rigidbodyProjectionHitCount < _internalRigidbodyProjectionHits.Length)
			{
				if (!hitRigidbody.GetComponent<NetworkedKinematicCharacterMotor>())
				{
			...
			out Vector3 creaseDirection)
		{
			...
			if (!characterIsStable || !currentHitIsStable || !previousHitIsStable)
			{
				...
				if (dotPlanes < 0.999f)
				{
					...
					Vector3 normalAOnCreasePlane =
						Vector3.ProjectOnPlane(currentHitNormal, tmpBlockingCreaseDirection).normalized;
					Vector3 normalBOnCreasePlane =
						Vector3.ProjectOnPlane(previousHitNormal, tmpBlockingCreaseDirection).normalized;
					float dotPlanesOnCreasePlane = Vector3.Dot(normalAOnCreasePlane, normalBOnCreasePlane);

					Vector3 enteringVelocityDirectionOnCreasePlane = Vector3
						.ProjectOnPlane(previousCharacterVelocity, tmpBlockingCreaseDirection).normalized;

					if (dotPlanesOnCreasePlane <=
						(Vector3.Dot(-enteringVelocityDirectionOnCreasePlane, normalAOnCreasePlane) + 0.001f) &&
						dotPlanesOnCreasePlane <=
						(Vector3.Dot(-enteringVelocityDirectionOnCreasePlane, normalBOnCreasePlane) + 0.001f))
					{
			...
			for (int i = 0; i < _rigidbodyProjectionHitCount; i++)
			{
				...
				if (bodyHit.Rigidbody && !_rigidbodiesPushedThisMove.Contains(bodyHit.Rigidbody))
				{
					if (_internalRigidbodyProjectionHits[i].Rigidbody != _attachedRigidbody)
					{
						...
						NetworkedKinematicCharacterMotor hitCharacterMotor =
							bodyHit.Rigidbody.GetComponent<NetworkedKinematicCharacterMotor>();
						...
						{
							NetworkedPhysicsMover physicsMover = bodyHit.Rigidbody.GetComponent<NetworkedPhysicsMover>();
							if (physicsMover)
							{
						...
						float characterToBodyMassRatio = 1f;
						{
							...
							else if (RigidbodyInteractionType == RigidbodyInteractionType.Kinematic &&
									 !hitBodyIsCharacter)
							{
						...
						{
							bodyHit.Rigidbody.AddForceAtPosition(velocityChangeOnBody, bodyHit.HitPoint,
			...
			out Vector3 velocityChangeOnBody)
		{
			...
			if (bodyVelocityMagnitudeOnHitNormal > characterVelocityMagnitudeOnHitNormal)
			{
				Vector3 relativeImpactVelocity =
					hitNormal * (bodyVelocityMagnitudeOnHitNormal - characterVelocityMagnitudeOnHitNormal);
		...
		public void EvaluateHitStability(Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint,
			...
			ref KinematicCharacterController.HitStabilityReport stabilityReport)
		{
			if (!_solveGrounding)
			{
			...
			if (LedgeAndDenivelationHandling)
			{
				...
				if (stabilityReport.LedgeDetected)
				{
					...
					stabilityReport.DistanceFromLedge = Vector3
						...
						.magnitude;
					stabilityReport.IsMovingTowardsEmptySideOfLedge = Vector3.Dot(withCharacterVelocity.normalized,
						stabilityReport.LedgeFacingDirection) > 0f;
				}
		...
		private void DetectSteps(Vector3 characterPosition, Quaternion characterRotation, Vector3 hitPoint,
			Vector3 innerHitDirection, ref KinematicCharacterController.HitStabilityReport stabilityReport)
		{
			int nbStepHits = 0;
			...
			Vector3 horizontalCharToHitDirection =
				Vector3.ProjectOnPlane((hitPoint - characterPosition), characterUp).normalized;
			Vector3 stepCheckStartPos = (hitPoint - verticalCharToHit) + (characterUp * MaxStepHeight) +
										(horizontalCharToHitDirection * CollisionOffset * 3f);
		...
		/// </summary>
		public void GetVelocityFromRigidbodyMovement(Rigidbody interactiveRigidbody, Vector3 atPoint, float deltaTime,
			out Vector3 linearVelocity, out Vector3 angularVelocity)
		{
			if (deltaTime > 0f)
			{
				...
				{
					NetworkedPhysicsMover physicsMover = interactiveRigidbody.GetComponent<NetworkedPhysicsMover>();
					if (physicsMover)
					{
		...
		private Rigidbody GetInteractiveRigidbody(Collider onCollider)
		{
			...
			if (colliderAttachedRigidbody)
			{
				if (colliderAttachedRigidbody.gameObject.GetComponent<NetworkedPhysicsMover>())
				{
			...
			float inflate = 0f, bool acceptOnlyStableGroundLayer = false)
		{
			...
			int nbHits = 0;
			int nbUnfilteredHits = Sandbox.Physics.OverlapCapsule(
				bottom,
				top,
			...
			out RaycastHit closestHit, RaycastHit[] hits, float inflate = 0f, bool acceptOnlyStableGroundLayer = false)
		{
			...
			// Capsule cast
			int nbHits = 0;
			int nbUnfilteredHits = Sandbox.Physics.CapsuleCast(
				bottom,
				top,
				Capsule.radius + inflate,
				direction,
			...
			out RaycastHit closestHit, RaycastHit[] hits, bool acceptOnlyStableGroundLayer = false)
		{
			...
			// Raycast
			int nbHits = 0;
			int nbUnfilteredHits = Sandbox.Physics.Raycast(
				position,
				direction,
				hits,
				distance,
